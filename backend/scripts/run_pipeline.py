...
def main() -> None:
    print("[1/9] DART/ECOS 원자료 수집 및 재무비율 계산 중...")
    ...
    print("[2/9] 거시요인(기준금리) 제거 중...")
    panel = macro_adjust.remove_macro_effect(panel)

    print("[3/9] 상장폐지 라벨 부여 중...")
    panel = labels.add_labels(panel)
    print(f"  -> label=1 로우 수: {int(panel['label'].sum())}")

    print("[4/9] 모델 학습 중 (서빙용, 전체 기간 데이터)...")
    metrics = model.train_model(panel)
    print(f"  -> {metrics}")

    print("[5/9] 시계열 학습/검증 분리 평가 중 (2022년까지 학습 -> 2023년 이후 검증)...")
    holdout = model.evaluate_temporal_holdout(panel, split_year=2022)
    print(f"  -> {holdout}")

    print("[6/9] Altman Z'-Score 벤치마크 비교 평가 중 (동일 검증 구간)...")
    altman = model.evaluate_altman_benchmark(panel, split_year=2022)
    print(f"  -> {altman}")

    print("[7/9] 로지스틱회귀+Altman 앙상블 평가 중 (동일 검증 구간)...")
    ensemble = model.evaluate_ensemble_benchmark(panel, split_year=2022)
    print(f"  -> {ensemble}")

    print("[8/9] 참고용 무작위 8:2 분할 평가 중 (시간 순서 무시, test_size=0.2)...")
    random_split = model.evaluate_random_split(panel, test_size=0.2)
    print(f"  -> {random_split}")

    print("[9/9] 위험점수 계산 및 저장 중...")
